...
import pocket_tts_timestamped.main
...
def test_client_disconnect_stops_the_generation(monkeypatch: pytest.MonkeyPatch):
    model = TTSModel.load_model()
    monkeypatch.setattr(pocket_tts_timestamped.main, "tts_model", model)
    voice_state = model.get_state_for_audio_prompt("alba")
    generation_steps = count_generation_steps(model, monkeypatch)

    stream = pocket_tts_timestamped.main.generate_data_with_state(TEXT, voice_state)
    next(stream)
